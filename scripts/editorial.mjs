...
const categoryLabel=id=>({music:'music',services:'creative practice',worlds:'independent brands'}[category(id)]);
const studyLabel=id=>({music:'sound',services:'making',worlds:'imagination'}[category(id)]);
...
export function homeDesign(html,worlds){
 ...
 .replace('<li><a href="#work">','<li><a href="#worlds">the collection</a></li><li><a href="#work">')
 .replace('<a href="#contact">contact</a>','<a class="nav-contact" href="#contact">say hello ↗</a>')
 .replace('<div class="hint" id="hint">pick a petal</div>',`<div class="hero-word" aria-hidden="true">florra</div><div class="hero-caption"><p class="micro">music · culture · possibility</p><p>be different<br>to be better.</p></div><a class="hero-index" href="#worlds">the collection ↓</a><div class="hint" id="hint">pick a petal. flick to explore.</div>`)
 .replace('<section id="worlds">',featured+'<section id="worlds">')
 .replace('<div class="eyebrow reveal">florra, from flower</div>',`<div class="section-kicker"><span>the collection</span><span>music, culture &amp; independent ideas</span></div><div class="section-heading"><div><div class="eyebrow">florra, from flower</div>`)
 .replace(/(<h1[^>]*>one stem\. many worlds\.<\/h1>)/,'$1</div>')
 .replace(/(<p class="lede reveal">[\s\S]*?<\/p>)/,'$1</div>')
 .replace('<div class="list" id="worldList">',`<div class="filters" role="group" aria-label="filter the collection">${[['all','all'],['music','music'],['services','creative services'],['worlds','brands & ideas']].map(([id,label])=>`<button type="button" data-filter="${id}" aria-pressed="${id==='all'}" aria-controls="worldList">${label}</button>`).join('')}</div><p id="filter-status" role="status" style="position:absolute;clip-path:inset(50%);width:1px;height:1px;overflow:hidden">${worlds.length} projects in view</p><div class="list" id="worldList">`)
...
export function chapterDesign(html,w,index){
 ...
 .replace('<div class="copy">',`${w.one?`<p class="intro">${escape(w.one)}</p>`:''}</div><div class="chapter-plate"><div class="plate-top"><span>florra / ${String(index+1).padStart(2,'0')}</span><span>${categoryLabel(w.id)}</span></div>${specimen(w.id)}<div class="plate-bottom"><span>independent by nature</span><span>↗</span></div></div></div><div class="chapter-content" id="chapter-content"><div class="copy">`)
